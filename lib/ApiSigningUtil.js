...
ApiSigningUtil.getSignatureBaseString = (baseProps) => {
    ...
    var paramsArray = json2array(defaultParams);
    ...
    var stringParams = paramsArray.sort().map(e => {
        if(e.length > 1 && e[1] == "") { 
            return e[0];
        } else {
            return e.join('=');
...
// convert json to 2 items array
function json2array(json) {
    // convert json to queryString to json 
    // to remove the sub-object
    // do not support property with sub-object as shown below
    // convert json from { "name" : { "name1" : "value1" } }
    //                to { "name" : "" }
    var safeQueryStringJson = qs.parse(qs.stringify(json));

    var result = [];
    var keys = Object.keys(safeQueryStringJson);
