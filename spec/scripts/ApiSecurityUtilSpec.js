...
describe('ApiSecurityUtil APEX L1 Test', function () {
    ...
    it('APEX L1 - If timestamp and nonce are overwritten by run-time values',
        function () {
            expect(secHeader.timestamp).to.exists;
            expect(secHeader.nonce).to.exists;
        });

    it('APEX L1 - Signature , Signature Method and BaseString are generated',
        function () {
...
describe('ApiSecurityUtil APEX L2 Test', function () {
    ...
    it('APEX L2 - If timestamp and nonce are overwritten by run-time default values',
        function () {
            expect(secHeader.timestamp).to.exists;
            expect(secHeader.nonce).to.exists;
        });

    it('APEX L2 - If app secret and pemfile are not overwritten by run-time default values',
        function () {
            expect(secHeader.secret).to.not.exists;
            expect(secHeader.pemFileName).
                to.
                not.
                equal(path.join(process.cwd(), './spec/cert/default.pem'));
        });
