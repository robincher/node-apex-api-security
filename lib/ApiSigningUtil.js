 ...
 * @param {string} reqProps.realm
 * @param {string} reqProps.httpMethod HTTP Verb
 * @param {string} [reqProps.secret] Apex App Secret (for L1)
 * @param {string} [reqProps.certFileName] Path to key file for signing request (L2)
 * @param {string} [reqProps.certString] String containing private key content in PEM format (L2)
...
ApiSigningUtil.getSignatureBaseString = (baseProps) => {
    ...
    winston.trace('signature url:: %s', signatureUrl);
    ...
    if (siteUrl.search != null && siteUrl.search.length > 0) {
        winston.trace('QueryString found in URL:: %s', siteUrl.search);
    ...
    if (!_.isNil(baseProps.queryString)) {
        winston.trace('QueryString passed into function:: %s', JSON.stringify(baseProps.queryString));
