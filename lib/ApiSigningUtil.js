...
ApiSigningUtil.getSignatureToken = (reqProps) => {
    ...
    let defaultParams = getDefaultParam(baseProps);
...
ApiSigningUtil.getSignatureBaseString = (baseProps) => {
    ...
    if (!_.isNil(baseProps.queryString)) {
        paramsArray = paramsArray.concat(parseParams(baseProps.queryString));
    }

    if (!_.isNil(baseProps.formData)) {
        paramsArray = paramsArray.concat(parseParams(baseProps.formData));
    }
...
function getDefaultParam(baseProps) {
    let defaultParams = {};
