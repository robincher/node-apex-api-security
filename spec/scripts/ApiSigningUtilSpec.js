...
describe('ApiSigning L1 HMACSHA256 Signature Test', function() {
    ...
    it('ApiSigning L1 HMACSHA256 - message is null or empty', function() {
        ...
        expect(
            ApiSigningUtil.getHMACSignature.bind(ApiSigningUtil, null, secret)).to.throw(expectedErrMsg);
        expect(
            ApiSigningUtil.getHMACSignature.bind(ApiSigningUtil, '', secret)).to.throw(expectedErrMsg);
    });

    it('ApiSigning L1 HMACSHA256 - Secret is null or empty', function() {
        expect(ApiSigningUtil.getHMACSignature.bind(ApiSigningUtil, message)).to.throw(expectedErrMsg);
        expect(
            ApiSigningUtil.getHMACSignature.bind(ApiSigningUtil, message,
                null)).to.throw(expectedErrMsg);
        expect(
            ApiSigningUtil.getHMACSignature.bind(ApiSigningUtil, message, '')).to.throw(expectedErrMsg);
    });
...
describe('ApiSigning L2 RSA256 Signature Test', function() {
    ...
    it('ApiSigning L2 RSA256 - PrivateKey is null or undefined', function() {
        expect(ApiSigningUtil.getRSASignature.bind(ApiSigningUtil, message)).to.throw(expectedGetErrMsg);
        expect(
            ApiSigningUtil.getRSASignature.bind(ApiSigningUtil, message, null)).to.throw(expectedGetErrMsg);
    });
...
describe('ApiSigning Signature Token Test', function() {
    ...
    it('Api Signed Signature Token - Basic L2 Test with cert string',
        function() {
            const keyString = ApiSigningUtil.getPrivateKeyFromPem(
                keyFile);
...
describe('ApiSigning (Deprecated) Token Test', function() {
    ...
    if (!liveTestToggle) {
        it('Api Signed Token - L0 Live Test', function(done) {
            ...
            ApiSigningUtil.sendRequest(liveUrl, null, null, null, httpMethod).then(function(data) {
        ...
        it('Api Signed Token - HMAC256 L1 Live Test', function(done) {
            ...
            ApiSigningUtil.sendRequest(liveUrl, token, null, httpMethod).then(function(data) {
                expect(data.status).to.equal(200);
                done();
            }).catch(function(error) {
                done(JSON.stringify(error));
