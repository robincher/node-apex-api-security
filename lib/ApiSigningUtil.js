...
ApiSigningUtil.getSignatureToken = (reqProps) => {
    ...
    let baseProps = {
        ...
        "queryString": reqProps.queryString,
...
ApiSigningUtil.getSignatureBaseString = (baseProps) => {
    ...
    if (siteUrl.search != null && siteUrl.search.length > 0) {
        winston.trace('QueryString:: %s', siteUrl.search);
    ...
    if (!_.isNil(baseProps.queryString)) {
        winston.trace('QueryString Param:: %s', JSON.stringify(baseProps.queryString));
 ...
 * @param {string} timestamp Timestamp
 *
 * @param {string} token HTTP Signature token to be append in Authorization header in HTTP
 * @public
 */
 ...
 *
 * @param {object} Sorted JSON object
 * @private
 */
